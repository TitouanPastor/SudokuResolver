...
package body tests_TAD_Coordonnee is
   ...
   -- test coord2index
   function test_Coordonnees_P5a return Boolean is
      c : Type_Coordonnee;
   begin
      c := construireCoordonnees(5,4);
      return coord2index(c) = 40;
   exception
      when others =>
         return False;
   end test_Coordonnees_P5a;

    -- test coord2index
   function test_Coordonnees_P5b return Boolean is
      index : Integer;
   begin
      index := 40;
      return index2coord(index) = construireCoordonnees(5,4);
   exception
      when others =>
         return False;

   end test_Coordonnees_P5b;
